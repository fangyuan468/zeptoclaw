...
mod tests {
    ...
    fn test_truncate_value_cjk() {
        // Euro signs are 3 bytes each.
        let s = "\u{20AC}\u{20AC}\u{20AC}\u{20AC}"; // 12 bytes
        let result = truncate_value(s, 7);
        assert!(result.ends_with("..."));
        // 7 bytes = 2 full Euro signs (6 bytes) + partial, so boundary at 6.
        assert_eq!(result, "\u{20AC}\u{20AC}...");
    }
}
